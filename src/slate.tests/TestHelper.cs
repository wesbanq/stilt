...
internal static class GoldenTestHelper
{
	...
	public static string GetGoldenPath(string slateFilePath, string suffix)
	{
		return slateFilePath + suffix;
	}

	/// <summary>
	/// Fails with full compiler/linker diagnostics when the build did not succeed cleanly.
	/// </summary>
	public static void AssertCompilationSucceeded(Compiler compiler, string suite, string shortName, string slatePath)
	{
		if (compiler.Files.Count == 0)
		{
			Assert.Fail(
				FormatFailureBanner(suite, shortName, slatePath, "No files were produced by the compiler.") +
		...
		body.AppendLine(FormatFailureBanner(suite, shortName, slatePath, "Compilation failed."));
		body.AppendLine();
	...
	private static string FormatFailureBanner(string suite, string shortName, string slatePath, string headline)
	{
		return $"""

			========== slate.Tests | {suite} | {headline} ==========
			Test file:  {shortName}
			Source:     {slatePath}
			""";
	}
		...
		string sourcePath)
	{
		...
		sb.AppendLine($"Source .slate: {sourcePath}");
		sb.AppendLine();
